...
public class PlacementProblem {
	...
	// Edge servers are end servers for now? Want gateways to forward requests to cloud
	// However they will need to perform computation afterwards when the cloud sends it back
	// But I think in this example the gateways perform computation anyway?
	...
	// One sensor and actuator per end device. Maybe if we make these mobile, they can model users?
	private static MyFogDevice addEnd(String endPartialName, int userId, String appId, int parentId) {
		...
		// inter-transmission time of sensor is PERIODIC. (DeterministicDistribution)
	...
	private static MyApplication createApplication(String appId, int userId) {
		...
		for(int id:idOfEndDevices) {
			...
			moduleDeadline.put("mainModule", getRandomValue(3.00, 5.00));
			Map<String,Integer>moduleAddMips = new HashMap<String,Integer>();
			moduleAddMips.put("mainModule", getRandomValue(0, 500));
